...
function ReportprovincesalesPageload(formrepid){
    
    var ipwithport = sessionStorage.Repserverip;
    var webservice = sessionStorage.Reppointedservice;
    managelogindata(formrepid);
...
function getdetailforreport(fromdate,todate,repotype){
    
     GetReportdata(fromdate,todate,repotype);
    
}


function GetReportdata(fromdate,todate,repotype) {
     var repo_ipwithport = sessionStorage.Repserverip;
     var repo_webservice = sessionStorage.Reppointedservice;
    var jason_data_object;
    try {
        
        if(repotype === 'regcusbrwis')
        {

            var settings = {
                "async": false,
                "crossDomain": true,
                "url": "http://" + repo_ipwithport + "/" + repo_webservice + "/webresources/GenericResource_Reportviewer/getreportdataregcus/?fromdate=" + fromdate+"&todate="+todate+"&reptype="+repotype,
                "contentType": "application/json",
                "method": "GET",
                "credentials": false,
                "headers": {
        ...
        else if(repotype === 'actvcusbrwis')
        {
            
            var settings = {
                "async": false,
                "crossDomain": true,
                "url": "http://" + repo_ipwithport + "/" + repo_webservice + "/webresources/GenericResource_Reportviewer/getreportdatactvcus/?fromdate=" + fromdate+"&todate="+todate+"&reptype="+repotype,
                "contentType": "application/json",
                "method": "GET",
                "credentials": false,
                "headers": {
        ...
        $.ajax(settings).done(function (response) {
            ...
            var jdset = jason_data_object;
            if (jdset !== null) {
                generatereportdataTable(jason_data_object);

            }
            if (jdset === null) {
                window.alert("No record to view !")
            }
        });
